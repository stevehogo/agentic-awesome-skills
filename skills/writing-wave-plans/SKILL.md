...
- Honor every dependency cell from the source; identify **parallel tracks** (waves sharing no files) and say when they may run concurrently. Parallelism *inside* a wave is a separate question, answered per wave file in Step 3 — see [`references/batching.md`](references/batching.md).
...
6. **Tasks** (`### Task <ID> — …`): Covers (ID, size, depends) · Files (exact paths; `Create:`/`Modify:` + line anchors where known) · Spec (values/quotes with source-section references; cite the feature's design-doc section where one exists) · Steps (inventory → change → verify → commit, per writing-plans granularity).
7. **`### Task WN-GATE`** — wave verification gate, in this order (field-tested; every step earned its place):
   ...
   **(f) Sync docs + gate commit** — wave Status (with evidence, see §4), README rollup row (outcome + env caveats + deferrals), any external checklist tooling reads; then a `docs(...)` gate commit.
