...
| **Per-wave verification gates** — build/tests → fresh-context audit → triage fix-vs-defer → in-tree remediation → tester-facing testing summary → **the wave's single commit** → context checkpoint | 3 |
...
4. **`## Status tracking — Wave N`** — its own `Status:` line + a checklist with one row per task + the gate. *This section is the single source of truth for the wave's task state* (the user-facing reason this format exists). **Tick with evidence:** a done task line carries a one-line delta + any deviation from the plan — no hash, because the wave's work stays uncommitted until its gate; the gate line records the **wave commit's subject**, verification results (build/tests/audit verdicts), what was **not verifiable in-session** (per the capability baseline), and every deferral with its new owner task ID. The subject rather than a hash is the anchor by construction: the status and rollup edits ride *inside* the wave commit, so no hash exists to write into them — recover it later with `git log --grep "W<N> —"`. That record — not the executor's memory — is what makes gates lossless compaction points.
5. Optional **task template** — the wave's repeated steps written once; per-task content is then only the deltas.
6. **Tasks** (`### Task <ID> — …`): Covers (ID, size, depends) · Files (exact paths; `Create:`/`Modify:` + line anchors where known) · Spec (values/quotes with source-section references; cite the feature's design-doc section where one exists) · Steps (inventory → change → verify → tick the wave checklist, per writing-plans granularity — no per-task commit).
