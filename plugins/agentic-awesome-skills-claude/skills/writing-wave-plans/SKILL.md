...
**Inherits `superpowers:writing-plans`** — read/apply its discipline first: assume zero context, bite-sized tasks (one 2–5-min action per step), exact file paths, complete code where it's knowable at plan time, exact verification commands with expected output, DRY/YAGNI, and its execution-handoff question at the end — with one deliberate departure: **commits are per wave, not per task** (Step 3, the wave gate). (If that skill isn't available in the environment, the summary in this sentence IS the discipline — proceed without it.) This skill adds the *folder structure, traceability, and verification layers* for efforts too big for one file.
...
| **Dependency graph** with parallel tracks — cross-wave in the README, **per-executor batches** inside each wave file ([`references/batching.md`](references/batching.md)) | 2–3 |
| **Per-wave verification gates** — build + the full test suites → fresh-context audit → triage fix-vs-defer → in-tree remediation → tester-facing testing summary → **the wave's single commit** → context checkpoint | 3 |
| **Two-rung test policy** — a task runs only the tests covering the files it touched; the gate runs the whole suite | 1, 3 |
...
5. **Probe test *selection*, not just test availability** — the per-task rung depends on it. Capture: the runner's exact subset syntax (`yarn vitest run <path>`, `npx jest <path>`, `pytest <path> -k <expr>`, `go test ./pkg/x/...`, `cargo test <module>`, `dotnet test --filter <expr>`); whether tests sit adjacent to their sources (so a file's covering tests are derivable) or in a separate tree that needs a stated naming map; and **how long the full suite takes** — that number decides whether scoping is worth doing at all (a 30-second suite *is* its own scoped rung; don't build selection machinery to save seconds). Run one selector for real and note the test count it collects. Record all of it on the Reality baseline's **Test scoping** line.
6. **Study prior art:** any earlier wave-plan folders in the target repo, plus archived plans for the same area — cite them as *decision history*, flagging where their values are superseded. For a filled-in example of the format itself, see [`references/exemplar-plan.md`](references/exemplar-plan.md). Scale the depth to the backlog — skim a prior plan's README for small efforts. Where a prior plan's formatting differs from `references/templates.md`, the templates are canonical.
7. Record findings as the README's **Reality baseline** section; if upstream docs/sheets have stale cells, add a **corrections table** (cell → stale claim → actual/authoritative value) so no executor follows the stale value. If authoritative docs are wrong about reality, the plan's first task (an `X0`) fixes them.
...
- Honor every dependency cell from the source; identify **parallel tracks** (waves sharing no files) and say when they may run concurrently — **each concurrent track runs in its own worktree**, created with `superpowers:using-git-worktrees` (one worktree per track branch), since each wave lands as a single commit and interleaved waves would smear each other's diff. Name each track's branch in the README so the executor creates the right worktrees and knows where the tracks reconverge. Parallelism *inside* a wave is a separate question, answered per wave file in Step 3 — see [`references/batching.md`](references/batching.md).
- Order risk deliberately — the riskiest, hardest-to-reverse work late, behind cheap reversible groundwork (e.g. the payment/checkout flow or a table-rewriting migration last; additive, zero-behavior-change scaffold or token waves first).
...
3. **`## Dependency graph`** — the wave's *verified* edges (not its declared ones), grouped into per-executor batches and rendered by batch: critical path, lanes, the ⚠ list of wrong/unowned/unverifiable edges, and an `Opens when` batch table. Method, drawing rules and the "this wave does not split" case: [`references/batching.md`](references/batching.md). **Write it when authoring the wave and re-run it before executing** — the second pass reads the finished Steps and reliably finds edges the first missed.
4. **`## Status tracking — Wave N`** — its own `Status:` line + a checklist with one row per task + the gate. *This section is the single source of truth for the wave's task state* (the user-facing reason this format exists). **Tick with evidence:** a done task line carries a one-line delta + any deviation from the plan — no hash, because the wave's work stays uncommitted until its gate; the gate line records the **wave commit's subject**, verification results (build, full-suite and audit verdicts), what was **not verifiable in-session** (per the capability baseline), and every deferral with its new owner task ID. The subject rather than a hash is the anchor by construction: the status and rollup edits ride *inside* the wave commit, so no hash exists to write into them — recover it later with `git log --grep "W<N> —"`. That record — not the executor's memory — is what makes gates lossless compaction points.
5. Optional **task template** — the wave's repeated steps written once; per-task content is then only the deltas.
6. **Tasks** (`### Task <ID> — …`): Covers (ID, size, depends) · Files (exact paths; `Create:`/`Modify:` + line anchors where known) · Spec (values/quotes with source-section references; cite the feature's design-doc section where one exists) · Tests (the exact selector for the tests covering *this task's* files + the expected result — or `none cover this surface` plus the degraded check, with the coverage gap re-homed onto a named QA-wave task) · Steps (inventory → change → verify (scoped tests + the cheap check) → tick the wave checklist, per writing-plans granularity — no per-task commit).
7. **`### Task WN-GATE`** — wave verification gate, in this order (field-tested; every step earned its place):
   **(a) Build + the FULL runnable test suites** — the whole suite, *not* the union of the tasks' scoped selections. Task-scoped greens don't compose: a per-task selection cannot see cross-task interaction, shared-fixture or ordering leakage, or the tests outside the paths that task touched — so this run, not the task ticks, is what authorizes the wave commit. Golden/snapshot artifacts (visual screenshots, API/contract fixtures, CLI golden files) are re-baselined only for *intended* changes, diffs eyeballed first; unrunnable rungs execute their degraded form and are recorded "not verified in-session", never claimed.
   ...
   **(d) Remediation** — fix in the working tree alongside the wave's other uncommitted work; no separate commit, and each fix is listed in the gate's status line (skip if nothing to fix).
   ...
   **(f) Sync docs, then commit the wave — once, with everything in it** — wave Status (with evidence, see §4), README rollup row (outcome + env caveats + deferrals), any external checklist tooling reads; *then* commit the wave's tasks, remediation, testing summary and status docs as a single `<type>(<scope>): W<N> — <theme> (<ID range>)`, its body listing one line per task ID. Docs go in before the commit, not after, so the wave lands as one reviewable unit and bisect steps wave-by-wave.
   **(g) Context checkpoint** — the executor stops after the wave commit and invites the user to `/compact` or `/clear` before the next wave. The executor can't compact context itself (`/compact` is user-invoked), but a passed gate is where compaction is lossless *by construction* — every fact the next wave needs is already on disk (wave Status sections, README rollup, git history), so the plan directs compaction to happen there rather than mid-wave.
8. Footer: "Wave N done when …" → link to next wave.
...
4. **Tooling check:** every workflow/skill/script invocation in gates uses names, args, and option keys that actually exist — **and is runnable in the execution environment** per the capability baseline (a script `package.json` doesn't define, or an e2e rung with no browsers installed, is a broken gate: write its degraded form instead). **Every per-task test selector must collect ≥1 test** — run each one (or the runner's list/collect-only mode) and record the count in the task's Tests line: a selector that matches nothing often still exits 0 (`go test -run Nope ./pkg` prints `no tests to run` and passes; `--passWithNoTests` does the same for jest/vitest), so a typo'd path or filter buys a green rung that tests nothing. Running it is the *only* proof here: the verifier's path check reads whole-code-span paths, not paths embedded inside a command, so a bad selector path is invisible to it. If no test covers the surface, say so and write the degraded check instead of a vacuous selector.
...
- **IDs are append-only.** Never renumber. Wave commit bodies list the task IDs they contain, so a renumber silently re-points history.
...
**Every mode starts in a worktree.** Before the first task of Wave 0, set up the effort's isolated workspace with **`superpowers:using-git-worktrees`** — it picks the directory (existing `.worktrees/` → `worktrees/` → CLAUDE.md preference → ask), verifies the directory is git-ignored, creates `git worktree add <path> -b <branch>`, runs project setup, and confirms a clean baseline test run. Record the resulting path + branch in the README's workspace line before executing. This is not optional polish: a wave plan makes one commit per wave over many waves, so the effort needs a branch that isn't disturbed by other work, and a baseline green run is what lets a gate blame its failures on the wave rather than on inherited breakage. Parallel tracks (Step 2) get **one worktree each**, off the same base commit, reconverging where the README says. After Final verification, retire the worktree(s) with `superpowers:finishing-a-development-branch` (merge / PR / cleanup). If either skill is unavailable, do the equivalent by hand — `git worktree add`, ignore-check, setup, baseline tests — and say so in the README.

Then offer the writing-plans choice: **1. Subagent-driven (this session)** — fresh subagent per task, review between tasks (reviewing the working tree — `git diff` since the previous wave commit, as tasks don't commit); **2. Wave-per-subagent (this session)** — a fresh subagent executes each wave and only gate results return to the orchestrator, so every wave starts with clean context (what compaction approximates, without needing it); or **3. Parallel session** — new session executes wave-by-wave with the gates as checkpoints (`superpowers:executing-plans`, or the environment's equivalent executing-plans skill, if available).
