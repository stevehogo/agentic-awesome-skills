...
- Honor every dependency cell from the source; identify **parallel tracks** (waves sharing no files) and say when they may run concurrently — concurrent tracks belong in separate worktrees or branches, since each wave lands as a single commit and interleaved waves would smear each other's diff. Parallelism *inside* a wave is a separate question, answered per wave file in Step 3 — see [`references/batching.md`](references/batching.md).
